...
if (Test-Path -LiteralPath $summaryPath) {
    ...
    $wrapped = [ordered]@{ endpoint_id = $EndpointId; profile = $Profile; run_id = $RunId; result = if ($exitCode -eq 0) { 'EJECUTADA' } else { 'FALLIDA' }; exit_code = $exitCode; metrics = $k6Summary.metrics; k6 = $k6Summary }
